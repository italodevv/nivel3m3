...
public class PessoaJuridicaDAO {
    ...
    public PessoaJuridica getPessoa(int idPessoa) {
        ...
        try {
            ...
            String sql = "SELECT p.nome, p.logradouro, p.cidade, p.estado, p.telefone, p.email, pj.cnpj " +
                    "FROM dbo.Pessoa p " +
                    "INNER JOIN dbo.PessoaJuridica pj ON p.idPessoa = pj.Pessoa_idPessoa " +
                    "WHERE p.idPessoa = ?";
    ...
    public List<PessoaJuridica> getPessoas() {
        ...
        try {
            connection = ConectorBD.getConnection();
            String sql = "SELECT p.idPessoa, p.nome, p.logradouro, p.cidade, p.estado, p.telefone, p.email, pj.cnpj FROM dbo.Pessoa p INNER JOIN dbo.PessoaJuridica pj ON p.idPessoa = pj.Pessoa_idPessoa";
            statement = connection.prepareStatement(sql);
            resultSet = statement.executeQuery();
    ...
    public int incluir(PessoaJuridica pessoa) {
        ...
        try {
            connection = ConectorBD.getConnection();
            String sqlPessoa = "INSERT INTO dbo.Pessoa (nome, logradouro, cidade, estado, telefone, email) VALUES (?, ?, ?, ?, ?, ?)";
            ...
            if (generatedKeys.next()){
                ...
                String sqlPessoaFisica = "INSERT INTO dbo.PessoaJuridica(Pessoa_idPessoa, cnpj) VALUES (?, ?)";
    ...
    public void alterar(PessoaJuridica pessoa) {
        ...
        try {
            ...
            String sqlPessoaJuridica = "UPDATE dbo.PessoaJuridica SET cnpj = ? WHERE Pessoa_idPessoa = ?";
    ...
    public void excluir(PessoaJuridica pessoa) {
        ...
        try {
            connection = ConectorBD.getConnection();
            String sqlPessoaJuridica = "DELETE FROM dbo.PessoaJuridica WHERE Pessoa_idPessoa = ?";
            statement = connection.prepareStatement(sqlPessoaJuridica);
